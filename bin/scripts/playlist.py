...
def main():
	...
	try:
		with open('scripts\sstv-playlist-config.json') as jsonConfig:
	...
	jsonGuide1 = getJSON("iptv.json", "https://iptvguide.netlify.com/iptv.json", "http://cdn.smoothstreams.tv/schedule/feed.json")
	jsonGuide2 = getJSON("tv.json", "https://iptvguide.netlify.com/tv.json", "http://sstv.fog.pt/feed.json")
...
def generatePlaylists(server, rtmp, service, streamQuality, minQuality, guideLookAheadMinutes, find, authSign, jsonGuide1, jsonGuide2):
	...
	if (foundChannels == 0):
		...
		writePlaylistFile("playlist\SmoothStreamsTV.xspf", xspfBodyTemplate.format(xspfTracks, xspfTracks2))
		writePlaylistFile("playlist\SmoothStreamsTV.m3u8", m3u8)
...
def generatePlaylistXML(server, rtmp, service, streamQuality, authSign, jsonGuide1, jsonGuide2, tvheadend):
	...
	if (foundPrograms == 0):
		...
		writePlaylistFile("playlist\SmoothStreamsTV-xml.m3u8", m3u8)
		writePlaylistFile("playlist\SmoothStreamsTV-xml.xml", xmlTemplate.format(xmlChannels, xmlPrograms))
